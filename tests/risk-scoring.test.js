...
assert.equal(context.RiskScoring.matchesBuiltIn(rule('selfByCharacter'), { Channel: 'Email', Source: 'alexander@example.com', Destination: 'alexandra@outside.test' }), true);
assert.equal(context.RiskScoring.matchesBuiltIn(rule('selfByCharacter'), { Channel: 'Email', Source: 'alice@example.com', Destination: 'bob@outside.test' }), false);
rule('selfByCharacter').settings.minimumCharacters = 8;
assert.equal(context.RiskScoring.matchesBuiltIn(rule('selfByCharacter'), { Source: 'alexander@example.com', Destination: 'alexandra@outside.test' }), false);
const broadcast = rule('emailBroadcastDomains');
assert.equal(context.RiskScoring.matchesBuiltIn(broadcast, { Size: '1E+3', Destination: 'a@one.test;b@two.test;c@three.test;d@four.test;e@five.test' }), true);
assert.equal(context.RiskScoring.matchesBuiltIn(broadcast, { Size: '12', Destination: 'a@gmail.com;b@gmail.com;c@yahoo.com;d@outlook.com;e@googlemail.com' }), true);
assert.equal(context.RiskScoring.matchesBuiltIn(broadcast, { Size: '12', Destination: 'a@gmail.com;b@gmail.com;c@gmail.com;d@gmail.com;e@gmail.com' }), true);
assert.equal(context.RiskScoring.matchesBuiltIn(broadcast, { Size: '', Destination: 'a@one.test;b@two.test;c@three.test;d@four.test;e@five.test' }), false);
broadcast.settings.minimumDomains = 3;
assert.equal(context.RiskScoring.matchesBuiltIn(broadcast, { Size: '12', Destination: 'a@one.test;b@two.test;c@three.test' }), true);
...
assert.equal(context.RiskScoring.matchesBuiltIn(rule('destinationCompetitor'), { Destination: 'Alice <alice@mail.metrobank.com>; https://gcash.com/pay' }), true);
assert.equal(context.RiskScoring.matchesBuiltIn(rule('destinationCompetitor'), { Destination: '<alice@mail.metrobank.com>' }), true);
assert.equal(context.RiskScoring.matchesBuiltIn(rule('destinationCompetitor'), { Destination: 'HTTPS://portal.securitybank.com.ph:443/login' }), true);
assert.equal(context.RiskScoring.matchesBuiltIn(rule('destinationCompetitor'), { Destination: 'person@notmetrobank.com' }), false);
const domainRows = [
  { Destination: 'one@unique.example; two@common.example' },
  { Destination: 'three@common.example' },
  { Destination: 'not-an-email' }
];
assert.equal(context.RiskScoring.matchesBuiltIn(rule('destinationDomainOnce'), domainRows[0], domainRows), true);
assert.equal(context.RiskScoring.matchesBuiltIn(rule('destinationDomainOnce'), domainRows[1], domainRows), false);
assert.equal(context.RiskScoring.matchesBuiltIn(rule('destinationEmailSubdomain'), { Size: '1E+3', Destination: 'person@regional.company.example' }), true);
assert.equal(context.RiskScoring.matchesBuiltIn(rule('destinationEmailSubdomain'), { Size: '12', Destination: 'person@a.example.com' }), false);
assert.equal(context.RiskScoring.matchesBuiltIn(rule('destinationEmailSubdomain'), { Size: '12', Destination: 'person@tenant.onmicrosoft.com' }), false);
assert.equal(context.RiskScoring.matchesBuiltIn(rule('destinationEmailSubdomain'), { Size: '', Destination: 'person@regional.company.example' }), false);
...
assert.equal(context.RiskScoring.matchesBuiltIn(sensitive, { Details: 'Confidential payroll', FileName: 'agenda.pdf' }), false);
sensitive.settings.keywords = 'board report, api key';
assert.equal(context.RiskScoring.matchesBuiltIn(sensitive, { Details: '', FileName: 'board_report.pdf' }), true);
